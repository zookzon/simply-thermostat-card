console.info("%c Simply-Thermostat-Card (yaml-ui compat v4.1) loaded", "color: lime; font-weight: bold");
...
class SimplyThermostatCard extends LitElementBase {
  ...
  static get styles(){ return css`
    ...
    .row > *{ flex:1 1 0; }
    .btn{ border-radius:10px; background:#2d2d2d; color:#9e9e9e; height:44px; display:flex; align-items:center; justify-content:center; cursor:pointer; min-width:0; padding:0 6px; }
  ...
  _renderHVACRow(list, current){ if(!list || !list.length) return html``; return html`
      <div class="row">
        ${list.map(m=>{ const active = String(m)===String(current); const cls = `btn ${active?'active '+m:''}`; const ic = MODE_ICONS[m] || 'mdi:thermostat';
          return html`<div class="${cls}" title="${m}" @click=${()=>this._setMode(m)}><ha-icon icon="${ic}"></ha-icon></div>`; })}
      </div>`;}
  _renderIconRow(type, list, current, iconFn){ if(!list || !list.length) return html``; return html`
      <div class="row">
        ${list.map(v=>{ const active = String(v)===String(current); const cls = `btn ${active?'active auto':''}`; const ic = iconFn(String(v));
          return html`<div class="${cls}" title="${v}" @click=${()=>this._setOption(type, v)}><ha-icon icon="${ic}"></ha-icon></div>`; })}
      </div>`;}
  _renderTextRow(type, list, current){ if(!list || !list.length) return html``; return html`
      <div class="row">
        ${list.map(v=>{ const active = String(v)===String(current); const cls = `btn ${active?'active auto':''}`;
          return html`<div class="${cls}" title="${v}" @click=${()=>this._setOption(type, v)}><span class="label">${String(v).replaceAll("_"," ")}</span></div>`; })}
      </div>`;}
  _renderChips(st, {fanModes, swingModes, presetModes}){
    const curT = st.attributes.current_temperature; const curH = st.attributes.current_humidity != null ? Math.round(st.attributes.current_humidity) : undefined;
    const left = html`<span class="chip blue"><ha-icon class="icon" icon="mdi:thermometer"></ha-icon>${curT!=null?`${curT}°C`:"-"}</span>
      <span class="chip"><ha-icon class="icon" icon="mdi:water"></ha-icon>${curH!=null?`${curH}%`:"-"}</span>`;
    const right = html`
      ${ (this._config.show_preset==="chip" && presetModes.length) ? html`<span class="chip purple click" @click=${()=>this._togglePanel("preset")} title="preset">${st.attributes.preset_mode || "-"}</span>` : ""}
      ${ (this._config.show_swing==="chip" && swingModes.length) ? html`<span class="chip yellow click" @click=${()=>this._togglePanel("swing")} title="swing"><ha-icon class="icon" icon="${swingIcon(st.attributes.swing_mode)}"></ha-icon>${st.attributes.swing_mode || "-"}</span>` : ""}
      ${ (this._config.show_fan==="chip" && fanModes.length) ? html`<span class="chip green click" @click=${()=>this._togglePanel("fan")} title="fan"><ha-icon class="icon" icon="${fanIcon(st.attributes.fan_mode)}"></ha-icon>${st.attributes.fan_mode || "-"}</span>` : ""}`;
    return html`<div class="chips"><div class="chips-left">${left}</div><div class="chips-right">${right}</div></div>`;
  }
  _renderPanel(type, list, current, iconFn){ if(!list || !list.length) return html``; return html`
      <div class="panel"><div class="panel-row">
        ${list.map(v=>{ const active = String(v)===String(current); const cls = `btn ${active?'active auto':''}`; const ic = iconFn(String(v));
          return html`<div class="${cls}" title="${v}" @click=${()=>this._setOption(type, v)}><ha-icon icon="${ic}"></ha-icon></div>`; })}
      </div></div>`;}
  _renderPanelText(type, list, current){ if(!list || !list.length) return html``; return html`
      <div class="panel"><div class="panel-row">
        ${list.map(v=>{ const active = String(v)===String(current); const cls = `btn ${active?'active auto':''}`;
          return html`<div class="${cls}" title="${v}" @click=${()=>this._setOption(type, v)}><span class="label">${String(v).replaceAll("_"," ")}</span></div>`; })}
      </div></div>`;}
  _togglePanel(which){ this._panelFan = which==="fan" ? !this._panelFan : false; this._panelPreset = which==="preset" ? !this._panelPreset : false; this._panelSwing = which==="swing" ? !this._panelSwing : false; this.requestUpdate(); }
  _adjustTemp(delta){ const st=this.hass.states[this._config.entity]; const cur=Number(st.attributes.temperature??st.attributes.target_temperature??st.attributes.target_temp_low??st.attributes.target_temp_high);
    if(Number.isFinite(cur)){ const next = cur + delta; this.hass.callService("climate","set_temperature",{entity_id:this._config.entity,temperature:next}); }}
  _setMode(m){ this.hass.callService("climate","set_hvac_mode",{entity_id:this._config.entity,hvac_mode:m}); }
  _setOption(type, val){ const svc = { fan_mode:"set_fan_mode", swing_mode:"set_swing_mode", preset_mode:"set_preset_mode" }[type]; if(!svc) return; this.hass.callService("climate", svc, { entity_id:this._config.entity, [type]:val }); }
...
window.customCards.push({
  ...
  description:"All-in-one card with flexible rows, chips, and bottom panels. v4.1"
});
